...
class ViewGeoNameTool extends CeemRadarTool {
  ...
  viewPanel.featureClassComboBox.addActionListener(new ActionListener {
    def actionPerformed(e: ActionEvent) { refreshFeatureCode() }
  })

  viewPanel.refreshButton.addActionListener(new ActionListener {
    def actionPerformed(e: ActionEvent){ refreshTable()  }
  })
  ...
  viewPanel.table.addMouseListener(new MouseAdapter {
    override def mouseClicked(e: MouseEvent){
      if (e.getClickCount == 2) {
        ...
        val worldWindow: WorldWindow = AppCeemRadarFrame.getAppCeemRadarFrame.getWwd
  ...
  def endFunction = (frame : ToolFrame) => {/* Ничего не реализовано по закрытии данного инструмента */ }
