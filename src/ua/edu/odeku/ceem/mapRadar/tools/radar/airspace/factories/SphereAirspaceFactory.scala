...
import gov.nasa.worldwind.render.airspaces.editor.{AirspaceEditor, SphereAirspaceEditor}
import gov.nasa.worldwind.render.airspaces.{Airspace, SphereAirspace}
...
class SphereAirspaceFactory(radar: Radar, wwd: WorldWindow, fitShapeToViewport: Boolean) extends AirspaceFactory(radar, wwd, fitShapeToViewport) {
	...
	private def initializeSphere(wwd: WorldWindow, sphere: SphereAirspace, fitShapeToViewport: Boolean) {
